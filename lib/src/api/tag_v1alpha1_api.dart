...
import 'package:halo_client/src/model/tag_list.dart';
...
class TagV1alpha1Api {
  ...
  const TagV1alpha1Api(this._dio, this._serializers);
  ...
  Future<Response<Tag>> createTag({ 
    ...
    final _path = r'/apis/content.halo.run/v1alpha1/tags';
    ...
    try {
      const _type = FullType(Tag);
      _bodyData = tag == null ? null : _serializers.serialize(tag, specifiedType: _type);
  ...
  Future<Response<void>> deleteTag({ 
    ...
    final _path = r'/apis/content.halo.run/v1alpha1/tags/{name}'.replaceAll('{' r'name' '}', encodeQueryParameter(_serializers, name, const FullType(String)).toString());
    final _options = Options(
      method: r'DELETE',
      headers: <String, dynamic>{
        ...?headers,
  ...
  /// * [onReceiveProgress] - A [ProgressCallback] that can be used to get the receive progress
  ///
  /// Returns a [Future] containing a [Response] with a [TagList] as data
  /// Throws [DioException] if API call or serialization fails
  Future<Response<TagList>> listTag({ 
    int? page,
    int? size,
    ...
    TagList? _responseData;

    try {
      final rawResponse = _response.data;
      _responseData = rawResponse == null ? null : _serializers.deserialize(
        rawResponse,
        specifiedType: const FullType(TagList),
      ) as TagList;
